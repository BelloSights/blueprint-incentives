...
contract DeployStorefront is Script {
    ...
    function run() external returns (address) {
        ...
        uint256 buybackBps = vm.envUint("BUYBACK_BPS");
        ...
        address blueprintWallet = vm.envAddress("BLUEPRINT_WALLET");
        if (paymentToken == address(0)) {
            revert("BLUEPRINT_TOKEN_ADDRESS is not set");
        }
        if (buybackBps == 0) {
            revert("BUYBACK_BPS is not set");
        }
        if (creatorBps == 0) {
        ...
        if (buybackWallet == address(0)) {
            revert("BUYBACK_WALLET is not set");
        }
        if (creatorWallet == address(0)) {
            revert("CREATOR_WALLET is not set");
        }
        if (blueprintWallet == address(0)) {
            revert("BLUEPRINT_WALLET is not set");
        }
